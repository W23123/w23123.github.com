...
### 简介
原文链接：http://www.cnblogs.com/waterystone/p/4920797.html

AQS即AbstractQueuedSynchronizer，它维护了volatile整型state(代表共享资源)和一个FIFO线程等待队列，state 的访问方式有3种：
- getState()
- setState()
- compareAndSetState()
...
- tryAcquire() 尝试获取资源，如果成功则返回。
- addWaiter () 将该线程加到队列的尾部，并标记为独占模式。
...
![aqs.png]({{site.baseurl}}/assets/img/aqs.png)
